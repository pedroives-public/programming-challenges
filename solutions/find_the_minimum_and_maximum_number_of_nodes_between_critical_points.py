...
class Solution:
    def nodesBetweenCriticalPoints(self, head: Optional[ListNode]) -> List[int]:
        localMin, localMax = head, head
        prev, curr = head, head.next

        posMin, posMax, flag, i = 0, 0, 0, 1
        while curr.next:
            i += 1
        ...
        ans = -1
        prev, curr, prevPos = head, head.next, 1
        while curr.next:
            i += 1
            if (prev.val < curr.val > curr.next.val) or (prev.val > curr.val < curr.next.val):
                if ans == -1:
                    ...
                    ans = min(ans, i - prevPos)
                    prevPos = i
            prev = prev.next
            curr = curr.next
        
        if ans == -1 or not (posMin and posMax):
            return [-1, -1]
            
        return [ans, posMax-posMin]